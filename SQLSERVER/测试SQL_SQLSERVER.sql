...
select id from a;
alter table asd alter column id decimal(18, 2)  null;
exec sp_rename 'test1.id', idd, 'COLUMN';
...
create unique index [table] on Student(S_StuNo) with (drop_existing=on);
create table table1(id int);
select id from a with (TABLOCKX);
...
/*纯数字*/create table cust1.[111] (id int, constraint ppp primary key (id));
...
CREATE TRIGGER production.trg_product_audit ON production.products AFTER
INSERT AS BEGIN
SET NOCOUNT ON;
...
CREATE TRIGGER production.tri_product_audit ON production.products AFTER
INSERT AS BEGIN
SET NOCOUNT ON;
INSERT INTO production.product_audits (product_id,product_name,brand_id,category_id,model_year,list_price,updated_at,operation)
SELECT i.product_id,product_name,brand_id,category_id,model_year,i.list_price,GETDATE(),'INS' FROM inserted i UNION ALL
...
select a from tb where a in(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);
create sequence user_camera_version start with 1 increment by 1 minvalue 1 no maxvalue ;
create sequence seq_user_camera_version start with 1 increment by 1 minvalue 1 no maxvalue;
create table mytab(id int not null , name TEXT);
alter table mytab alter column name TEXT not null;
alter table members add name TEXT;
create table mytab(id int not null , name IMAGE);
alter table mytab alter column name IMAGE not null;
alter table members add name IMAGE;
create table mytab(id int not null);
alter table mytab alter column name int not null;
