...
--【审核规则】修改列定义操作
/*修改列定义操作*/alter table T3 alter column ID set data type clob;
/*修改列定义操作*/alter table T1 rename column A to A1
...
/*X-创建数据库对象时未指定表空间*/create table a(id int) in tablespace_name;
...
--【审核规则】删除索引操作	
/*删除索引操作	*/DROP INDEX index_name;

--【审核规则】在SQL中使用悲观锁	
/*在SQL中使用悲观锁	*/select id from a for update;

/*X-在SQL中使用悲观锁	*/select id from a;
...
/*不建议使用COUNT(col)或COUNT(常量)*/select count(4) from stu;
...
--【审核规则】SQL指定RR隔离级别	
/*SQL指定RR隔离级别*/select * from stu with RR;

--【审核规则】SQL指定UR隔离级别	
/*SQL指定UR隔离级别*/select * from stu with UR;